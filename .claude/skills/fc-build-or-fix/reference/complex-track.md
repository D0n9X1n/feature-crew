...
6. **Cross-audit the plan.** The plan is a model-authored hard-gate artifact, so it gets what the spec got: a review-family model in one-clue mode. Focus on tasks that trace to no spec requirement, requirements with no task, placeholder steps, and names that drift between tasks. Address CRITICAL findings before asking for approval.
...
9. **Per-task QA, one-clue mode.** `fc-qa-spec` first, then `fc-qa-code`. In Complex, `fc-qa-code` skips spec compliance because `fc-qa-spec` already covered it.
10. **Tech Lead final.** Dispatch `fc-tech-lead` with spec, plan, full diff, and task summaries. Hard gate before merge.
11. **Cost telemetry.** One line appended to the PR description.
